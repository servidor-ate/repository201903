	...
	public int guardarUsuarioPaciente(Personamast objDao) {
		...
		try{
			...
			if(hallado){
				...
				objSave.setUltimaFechaModif(objDao.getUltimaFechaModif());
				result = -1;
				personamastDao.actualizar(objSave);
				IdPersonaAux = objSave.getPersona();

			}else{
				if(halladoPersona){
					//solo actualizar algunos datos
					...
					String nombreCompleto =
							(objDao.getApellidoPaterno()+"").trim()+" "+
									(objDao.getApellidoMaterno()+"").trim()+" "+
									(objDao.getNombres()+"").trim();

					//OBS *** cota 100
					if(nombreCompleto.length() > 100){
						nombreCompleto  = nombreCompleto.substring(0,100);
					}
					objDao.setNombreCompleto(nombreCompleto);
					objDao.setBusqueda(nombreCompleto);
					objDao.setOrigen(Constant.APP_CODIGO);
					IdPersonaAux = personamastDao.guardarPersonamast(objDao);
				}
...
//				}
				...
				if(agente != null){
					...
					List<SgAgente> listaAgentePerf = sgAgenteDao.listarElementos(objAgentePerf,false) ;

					SgAgente agentePerf = null;
					if(UtilesCommons.noEsVacio(listaAgentePerf)){
						agentePerf = listaAgentePerf.get(0);
					}else{
				...
				//SI todo va BIEN, devolver el ID de la persona
				result = IdPersonaAux;
